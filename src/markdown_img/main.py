...
class Main:
    ...
    def getCopyFilePath(self, fileName: str):
        ...
        SysConfig = Config()
        return (
            SysConfig.getMarkdownImgDirPath() + SysConfig.getPathSplit() + copyFileName
        )
    ...
    def __replaceWithRelativeMDFile(self, localImages: set, originalFile: str):
        ...
        with open(file=originalFile, mode="r", encoding="UTF-8") as fread:
            for line in fread:
                for localImg in localImages:
                    ...
                    relativeLocalImage = (
                        sysConfig.getImagesDirPath()
                        + sysConfig.getPathSplit()
                        + imgName
                    )
        ...
        # 保存原始文件备份
        backupFilePath = (
            sysConfig.getBackUpDirPath()
            + sysConfig.getPathSplit()
            + FileTools.getFileName(originalFile)
        )
    ...
    def dealUserException(self, userExp: UserException):
        ...
        if userExp.getErrorCode() == UserException.CODE_NO_SMMS_TOKEN:
            ...
            print(
                self.globalization.getText("update_image_fail").format(
                    currentImgService
                ),
    ...
    def showConfigFile(self):
        ...
        if sys.platform == "win32":
            # 如果是 windows 系统，展示文件链接
            configFilePath: str = sysConfig.getConfigFilePath()
            configFileName: str = FileTools.getFileName(configFilePath)
            print(FileTools.getWindowsTerminalPathLink(configFileName, configFilePath))
        else:
            print(sysConfig.getConfigFilePath())
            ...
            if os.path.isfile(dir):
                if self.isOrigMdFile(dir):
                    ...
                    if os.path.exists(copyFilePath):
                        if (
                            refresh
                            and TimeHelper.compareTwoFilsLastModifyTime(
                                dir, copyFilePath
                            )
                            > 0
                        ):
                    ...
                    if sys.platform == "win32":
                        ...
                        sourceLink = FileTools.getWindowsTerminalPathLink(
                            dir, absSourcePath
                        )
                        targetFileName = FileTools.getFileName(copyFilePath)
                        targetLink = FileTools.getWindowsTerminalPathLink(
                            targetFileName, copyFilePath
                        )
                        print(
                            self.globalization.getText(
                                "deal_success_with_detail"
                            ).format(sourceLink, targetLink)
                        )
                    else:
    ...
    def recoveryImgsInMarkdown(self, copyFilePath: str, orignalFile: str):
        ...
        if len(copyImages) == len(orignalImages):
            ...
            print(
                self.globalization.getText("images_number_is_not_equal").format(
                    orignalFile
                )
            )
            return False
    ...
    def changeImgService(self, selectedService):
        if not ImgServiceManager.isValidImgServiceFlag(selectedService):
            print(
                self.globalization.getText("not_support_img_service"), selectedService
            )
            return False
        sysConfig = Config()
    ...
    def changeToken(self, imgService):
        if imgService != "this" and (
            not ImgServiceManager.isValidImgServiceFlag(imgService)
        ):
    ...
    def changeImgServiceOption(self, imgServiceFlag, options):
        ...
        if imgServiceFlag == Config.IMG_SERVICE_QCLOUD:
            try:
                ...
                self.dealUserException(e)
            if (
                Config.QCLOUD_INFO_DES_DIR in options
                and options[Config.QCLOUD_INFO_DES_DIR]
            ):
                qCloudInfo[Config.QCLOUD_INFO_DES_DIR] = options[
                    Config.QCLOUD_INFO_DES_DIR
                ]
    ...
    def changeMainPrams(self, params: dict):
        ...
        for key, value in params.items():
            if key == Config.PARAM_URL_ENCODE_MODE:
                if value in (
                    Config.URL_ENCODE_MODE_NONE,
                    Config.URL_ENCODE_MODE_ONLY_SPACE,
                    Config.URL_ENCODE_MODE_STANDARD,
                ):
                    sysConfig.setConfigParam(Config.PARAM_URL_ENCODE_MODE, value)
                else:
                    exp = UserException(
                        UserException.CODE_ERROR_INPUT,
                        self.globalization.getText("input_error_and_hint").format(
                            value
                        ),
                    )
                ...
                if value in (
                    Config.COMPRESS_ENGINE_GIL,
                    Config.COMPRESS_ENGINE_TIYPNG,
                    Config.COMPRESS_ENGINE_NONE,
                ):
                    sysConfig.setConfigParam(Config.PARAM_COMPRESS_ENGINE, value)
                else:
                    exp = UserException(
                        UserException.CODE_ERROR_INPUT,
                        self.globalization.getText("input_error_and_hint"),
                    )
            else:
                pass
        sysConfig.writeMainConfig()
    ...
    def printSysInfo(self):
        ...
        print(
            "{}{}{}".format(
                self.globalization.getText("program_version"),
        ...
        try:
            ...
            print(
                "{}{}{}".format(
                    self.globalization.getText("working_language"),
                    self.globalization.getText("colon"),
                    languageText,
                )
            )
            print(
                "DEBUG{}{}".format(
                    self.globalization.getText("colon"),
                    sysConfig.getConfigParam(Config.PARAM_DEBUG),
                )
            )
            imgService = sysConfig.getConfigParam(Config.PARAM_IMG_SERVICE)
            print(
                "{}{}{}".format(
                    self.globalization.getText("current_used_image_bed"),
                    self.globalization.getText("colon"),
                    imgService,
                )
            )
            ...
            if urlEncodeMode == Config.URL_ENCODE_MODE_NONE:
                ...
                urlEncodeModeText = self.globalization.getText("use_only_for_spaces")
            else:
            ...
            print(
                "{}{}{}".format(
                    self.globalization.getText("whether_to_use_url_encoding"),
                    self.globalization.getText("colon"),
                    urlEncodeModeText,
                )
            )
            ...
            print(
                "{}{}".format(
                    self.globalization.getText("image_bed_configs"),
                    self.globalization.getText("colon"),
                )
            )
    ...
    def listConfigBackup(self) -> None:
        ...
        print(
            "{:<25s} {:<10s}".format(
                self.globalization.getText("config_name"),
                self.globalization.getText("create_time"),
            )
        )
    ...
    def changeConfig(self, configFileName: str) -> None:
        """使用指定配置覆盖当前配置"""
        sysConfig = Config.getInstance()
        configFile: str = ConfigBackup.getConfigBackupFile(sysConfig, configFileName)
        if not os.path.exists(configFile):
            print(self.globalization.getText("no_config_file"))
            return
